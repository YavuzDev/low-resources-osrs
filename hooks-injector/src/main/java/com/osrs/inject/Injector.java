...
public class Injector {
    ...
    public void loadMixin(Class<?> mixinClass) {
        ...
        ClassHook classHook = hooks.getClasses().get(mixin.value());
        if (classHook == null) {
    ...
    private void addReplaceMethodAdapter(Mixin mixin, ClassHook classHook, Method method) {
        var replace = method.getAnnotation(Replace.class);
        var isStatic = method.isAnnotationPresent(Static.class) || Modifier.isStatic(method.getModifiers());
        String owner;
        String name;
        String desc;
        if (isStatic) {
            var methodHook = hooks.getStaticMethod(replace.value());
            if (methodHook == null) {
            ...
            var methodHook = classHook.getMethod(replace.value());
            if (methodHook == null) {
        ...
        var newType = Type.getMethodDescriptor(method);

//        var indexOfOriginal = desc.indexOf(")");
//        var lastCharacterOriginal = desc.charAt(indexOfOriginal - 1);
//
//        var indexOfNew = newType.indexOf(")");
//        var lastCharacterNew = newType.charAt(indexOfNew - 1);

        var builder = new StringBuilder(newType);
//        if (lastCharacterNew != lastCharacterOriginal) {
//            builder.insert(indexOfNew, lastCharacterOriginal);
//        }
    ...
    private void addGetterAdapter(Mixin mixin, ClassHook classHook, Method method) {
        ...
        if (isStatic) {
            ...
            fieldDesc = fieldHook.getType();
            fieldMultiplier = null; //TODO check this
        } else {
    ...
    private void addSetterAdapter(Mixin mixin, ClassHook classHook, Method method) {
        ...
        Number inversedMultiplier = null; //TODO CHECK THIS
...
//        }
    ...
    private void addInjectCallbackAdapter(Mixin mixin, ClassHook classHook, Method method) {
        ...
        preCopyAdapterGroup.addAdapter(classHook.getName(), delegate ->
                new AddInjectCallbackAdapter(delegate, methodHook.getName(), methodHook.getType(),
